...
train_dataset = Date_Loader(path = "/home/yijiahe/you/dataset/large_yes/train.txt") 
...
test_dataset = Date_Loader(path = "/home/yijiahe/you/dataset/large_yes/val.txt",train_flag = False)
...
net.classifier[3] = nn.Linear(num_ftrs, 5)  #类别数量
...
cirterion = CELoss(label_smooth=0.05, class_num=5)   #类别数量
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if (epoch % 10 == 0) or (epoch >= (epochs - 30)):
            ...
            torch.save(net.state_dict(), "./large_yes/mobile_best_%s_train_acc_%s_test_acc_%s_loss_%s.pth" % (epoch, train_acc.cpu().detach().numpy(), test_acc.cpu().detach().numpy(), (test_loss.sum() / test_total).cpu().detach().numpy()))
