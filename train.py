...
test_dataset = Date_Loader(path="/home/yijiahe/you/dataset/cls9/val.txt",train_flag = False)#####测试数据集的地址
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if epoch % 10 == 0 and epoch > 20:
            torch.save(net.state_dict(), "./weight_c9/resnet18_%s.pth" % (epoch))

        if (cur_acc >= acc) and (epoch >= (epochs - 10)):
            torch.save(net.state_dict(), "./weight_c9/resnet18_best_%s_acc_%s.pth" % (epoch,cur_acc.cpu().detach().numpy()))
            acc = cur_acc
